...
module.exports = {
  ...
  async getProductPrice() {
    const productPriceString = await this.grabTextFrom({ xpath: '//*[@id="content"]/div[1]/div[2]/div/div[1]/span[1]' });
    const colorPriceString = await this.grabTextFrom({ xpath: "//a[contains(text(),'Black') and contains(text(), '(+$') and contains(@class, 'sbSelector')]" });
  ...
  async getCheckoutPrice() {
    const checkoutPriceString = await this.grabTextFrom({ xpath: '//*[@id="collapse-checkout-confirm"]/div/div[1]/table/tfoot/tr[3]/td[2]' });
    return checkoutPrice = I.parsePrice(checkoutPriceString);
  },

  async getShippingPrice() {
    const shippingPriceString = await this.grabTextFrom({ xpath: '//*[@id="collapse-checkout-confirm"]/div/div[1]/table/tfoot/tr[2]/td[2]' });
